...
function request() {
  const stage = { stageId: hash("1"), index: 0, chainId: 196 as const,
    predecessorStageId: null, calls: [{ adapter: { id: "okx.swap", version: 1 }, target: address("3"),
      targetRuntimeCodeHash: hash("2"), calldata: "0x12345678", nativeValueAtomic: "0", gasLimit: 300_000,
  ...
  return { chainId: 196 as const, blockNumber: "123", blockHash: hash("6"), owner: address("1"),
    executor: address("2"), stage, compiled: { stageId: stage.stageId, chainId: 196 as const,
      calls: [{ adapterKey: hash("7"), target: stage.calls[0]!.target,
        targetRuntimeCodeHash: stage.calls[0]!.targetRuntimeCodeHash,
        data: stage.calls[0]!.calldata, valueAtomic: "0", gasLimit: 300_000,
        approvals: stage.calls[0]!.approvals, quoteHash: hash("8"), expiresAtSec: 2_000_000_000 }],
      refundTokens: stage.refundTokens, quoteHash: hash("9"), expiresAtSec: 2_000_000_000 } };
}
...
describe("general asset remote stage replay", () => {
  it("rejects any compiled call that differs from the committed stage before starting replay", async () => {
    const rpc = vi.fn();
    const input = request();
    input.compiled.calls[0]!.data = "0x87654321";
