import { GeneralAssetStageV1Schema, commitment } from "@cobia/domain";
import {
...
const CompiledSchema = z.object({
  stageId: HashSchema, chainId: z.union([z.literal(1), z.literal(196)]), adapterKey: HashSchema,
  ...
  approvals: GeneralAssetStageV1Schema.shape.approvals,
...
async function send(rpc: Rpc, from: Address, to: Address, data: Hex, value = 0n, gas = 1_000_000) {
  const result = await rpc("eth_sendTransaction", [{ from, to, data, value: toHex(value), gas: toHex(gas) }]);
...
export async function replayGeneralAssetStageOnFork(inputValue: unknown, rpc: Rpc) {
  ...
  if (input.chainId !== stage.chainId || compiled.chainId !== stage.chainId ||
      compiled.stageId !== stage.stageId || compiled.target !== stage.target ||
  ...
  const code = await rpc("eth_getCode", [stage.target, "latest"]);
  if (typeof code !== "string" || code === "0x" || keccak256(code as Hex) !== stage.targetRuntimeCodeHash) {
    throw new Error("General asset replay target code changed");
  }
  const tokens = [...new Set(stage.refundTokens)].sort();
  ...
  try {
    ...
    transactions.push(await send(rpc, input.executor, compiled.target, compiled.data,
      BigInt(compiled.valueAtomic), compiled.gasLimit));
    for (const approval of compiled.approvals) transactions.push(await send(
  ...
  const endingAllowances = await Promise.all(compiled.approvals.map(async ({ token, spender }) =>
