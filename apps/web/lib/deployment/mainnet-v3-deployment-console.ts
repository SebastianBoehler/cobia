...
import type { buildAgentExecutorDeploymentPlanV3 } from "./agent-executor-v3-plan";

type Plan = ReturnType<typeof buildAgentExecutorDeploymentPlanV3>;
const GAS_LIMITS = [2_200_000n, 3_500_000n] as const;

function safeJson(value: unknown): string {
  return JSON.stringify(value).replaceAll("<", "\\u003c");
}

export function renderMainnetV3DeploymentConsole(input: { plan: Plan; maxFeePerGas: bigint }) {
  const { plan } = input;
  if (plan.chainId !== 196 || plan.version !== 3 || plan.deployments.length !== 2 ||
    plan.deployments[0]?.label !== "deploy-risk-manager" ||
    plan.deployments[1]?.label !== "deploy-executor-v3" || input.maxFeePerGas <= 0n) {
    throw new Error("Unsafe V3 mainnet deployment console input");
  }
  ...
  const consoleInput = safeJson({
    ...
    sequence: plan.deployments.map((deployment, index) => ({
      ...deployment, kind: "create", gas: toHex(GAS_LIMITS[index]!),
    })),
  });
