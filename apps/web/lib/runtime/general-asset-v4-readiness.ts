import { isAddressEqual, keccak256, parseAbi, stringToHex,
...
export async function assertGeneralAssetV4Ready(input: { client: Client; config: Config;
  ...
  const [executorHash, registryHash, riskHash, registry, riskManager, riskExecutor,
    verifier, riskPaused, accessMode, registryPaused, active, limits] = await Promise.all([
    ...
    read(input.config.registry, "paused"), read(input.config.registry, "isActive",
      [keccak256(stringToHex("okx.swap@1")), input.target, input.selector]),
  ...
  if (executorHash !== input.config.executorCodeHash || registryHash !== input.config.registryCodeHash ||
      ...
      registryPaused !== false || active !== true || tuple[0] !== 100_000_000_000n ||
