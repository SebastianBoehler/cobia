  ...
  return <div className="solver-directory">{solvers.map((solver) => {
    ...
    return <article key={solver.id}>
      ...
      <div className="solver-directory__columns">
        <div><h3>Declared capabilities</h3><ul>{solver.declaredCapabilities.map((capability) => <li key={capability}>{capability}</li>)}</ul></div>
        <div><h3>Verifier-derived results</h3><dl><div><dt>Accepted</dt><dd>{solver.stats.accepted}</dd></div><div><dt>Rejected</dt><dd>{solver.stats.rejected}</dd></div><div><dt>Wins</dt><dd>{solver.stats.wins}</dd></div></dl></div>
      </div>
      <Link href={`/solvers/${solver.id}`}>View solver history <ArrowRight aria-hidden="true" size={15} /></Link>
    </article>;
  })}</div>;
}
