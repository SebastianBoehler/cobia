...
function message(input: unknown): string {
  return typeof input === "object" && input && "message" in input && typeof input.message === "string"
    ? input.message
    : "Could not unlock the purchased route.";
}
...
export function RouteAccessView({ routeId }: { routeId: string }) {
  ...
  async function unlock(): Promise<void> {
    ...
    setError(undefined);
    try {
      if (!wallet.account) throw new Error("Connect the buyer wallet to unlock this route.");
      ...
      if (typeof signature !== "string") throw new Error("Wallet returned an invalid route-access signature.");
      ...
      setError(cause instanceof Error ? cause.message : "Could not unlock the purchased route.");
    } finally {
      setPending(false);
    }
  }
  ...
  return (
    <main className={styles.page}>
      <section className={styles.gate}>
        ...
        <h1>Unlock your purchased route</h1>
        <p>The exact solver path is private. Sign a read-only message with the buyer wallet to recover it.</p>
        ...
        <button className="button button--primary" onClick={unlock} disabled={pending || !wallet.account}>
          ...
          {wallet.account ? "Sign and show route" : "Connect buyer wallet first"}
        </button>
      </section>
    </main>
  );
