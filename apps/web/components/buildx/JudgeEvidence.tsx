...
const evidence = [
  {
    label: "Product surface", title: "Cobia on X Layer", status: "Live product",
  ...
  {
    label: "Mainnet runtime", title: "Executor V3 deployment", status: "Deployed",
  ...
  {
    label: "Eligibility deployment", title: "X Layer testnet", status: "Deployed",
  ...
  {
    label: "Ecosystem attribution", title: "Registered Builder Code", status: "Attributable",
  ...
  {
    label: "Implementation", title: "Public source", status: "Public source",
  ...
  {
    label: "Paid agent commerce", title: "Ethy AI via x402", status: "Pending canary",
...
export function JudgeEvidence() {
  return <main className={styles.page} id="main-content">
    <section className={styles.hero}>
      <div className={styles.heroInner}>
        <div className={styles.heroCopy}>
          <p className={styles.kicker}><span aria-hidden="true" /> BuildX AI Season · Judge evidence</p>
          ...
          <time dateTime="2026-08-21">Evidence snapshot · 21 August 2026</time>
        </div>
        <div aria-label="Cobia verification sequence" className={styles.signal}>
          ...
          <div className={styles.signalVerdict}>
            ...
            <small>AI receives no signing key or production send method.</small>
          </div>
        </div>
      </div>
    ...
    <section className={styles.boundary} id="boundary">
      <header><p>The boundary</p><h2>Creative search.<br />Verified authorization.</h2></header>
    ...
    <section className={styles.evidence} id="evidence">
      <header className={styles.sectionHeader}>
        <div><p>Public evidence</p><h2>Claims that resolve to something inspectable.</h2></div>
      ...
      <div className={styles.evidenceList}>{evidence.map((item) => <article key={item.title}>
        <div className={styles.evidenceIndex}><span>{item.label}</span><strong>{item.status}</strong></div>
    ...
    <section className={styles.rubric}>
      <header className={styles.sectionHeader}><div><p>Why Cobia</p><h2>Built around the judging lens.</h2></div></header>
      <dl>{rubric.map(([term, detail]) => <div key={term}><dt>{term}</dt><dd>{detail}</dd></div>)}</dl>
    </section>

    <section className={styles.roadmap} id="roadmap">
      <header className={styles.sectionHeader}>
        <div><p>Roadmap</p><h2>Proof first. Supply next. Distribution follows.</h2></div>
        <span>Targets, not shipped claims</span>
      </header>
      <ol>{roadmap.map(([time, title, detail], index) => <li key={time}>
        <div className={styles.roadmapMarker}><span>{String(index + 1).padStart(2, "0")}</span><i aria-hidden="true" /></div>
        <div><small>{time}</small><h3>{title}</h3><p>{detail}</p></div>
      </li>)}</ol>
    </section>

    <section className={styles.close}>
      <div><p>Transaction firewall for the agent economy</p><h2>State the outcome.<br />Keep the keys.</h2></div>
