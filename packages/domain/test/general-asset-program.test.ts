...
function stage(index: number, predecessorStageId: string | null) {
  return {
    ...
    predecessorStageId,
    calls: [{ adapter: { id: index === 0 ? "lifi.bridge" : "okx.swap", version: 1 },
      target: router, targetRuntimeCodeHash: hash("9"), calldata: "0x12345678",
      nativeValueAtomic: "0", gasLimit: 300_000, approvals: [{ token: index === 0 ? weth : usdt0,
        spender: router, maximumAtomic: "1000000" }] }],
    input: { token: index === 0 ? weth : usdt0, maximumAtomic: "1000000",
      maximumUsdE8: "100000000", identityEvidenceHash: index === 0 ? hash("d") : hash("e"),
      valuationEvidenceHash: hash("f") },
    outputs: [{ token: usdt0, minimumIncreaseAtomic: "900000", identityEvidenceHash: hash("e") }],
